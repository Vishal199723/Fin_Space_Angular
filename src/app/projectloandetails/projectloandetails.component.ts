...
@Component({
  selector: 'app-projectloandetails',
  templateUrl: './projectloandetails.component.html',
  styleUrls: ['./projectloandetails.component.css'],
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: { displayDefaultIndicatorType: false }
  }]
})
export class ProjectloandetailsComponent implements OnInit {
  ...
  constructor(private messageService: MessageService,private businessService: BusinessLoanServiceService, private spinner: NgxSpinnerService, private router: Router,
    private workflowtrnsservice: WorkflowtransService, private route: Router,private mailservice: MailService,private httpservice:HttpClient) {
  ...
  backendServices() {
    ...
    this.businessService.getBusinessLoanDetailsUser(this.userdetailsfor).subscribe((response: any) => {
      ...
      if(this.alldetails.fixedAssetDetails !=null || this.alldetails.fixedAssetLoanDetails !=undefined){
  ...
  projectloan(stepper: MatStepper) {
    this.submitted = true;
    stepper.next();
  }
  ...
  //       TicketId: this.customerdata.complaintId,
