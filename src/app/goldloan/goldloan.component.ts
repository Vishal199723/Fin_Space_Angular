...
export class GoldloanComponent implements OnInit {
  ...
  get g() { return this.goldloanForm.controls; }
  bankdocs: any = [];
  Nomineetype: any;
  Nagedob: any
  minormiddname: any;
  ...
  goldloanForm = new FormGroup({
    customerid: new FormControl(),
    requiredamount: new FormControl(),
    overdrafts: new FormControl(),
    goldamt: new FormControl(),
    goldtotal: new FormControl(),
    goldvalue: new FormControl(),
    Mon: new FormControl(),
    Quar: new FormControl(),
  });
  SendCopytoSP: any = false
  files1: any;
  ...
  constructor(private messageService: MessageService, private formBuilder: FormBuilder, private http: HttpClient, private countryService: CountryService,
    ...
    this.goldloanForm = formBuilder.group({
      requiredamount: ['', Validators.required],
      overdrafts: ['', Validators.required],
      goldamt: ['', Validators.required],
      goldtotal: ['', Validators.required],
    ...
    else if (this.editformname == "gold") {
      this.GetEditData();
      this.enablepreview = true;
  ...
  ngOnInit() {
    ...
    var LoanType2 = {
      LoanTypeId: 3,
      LoanTypeName: "Bullet Loan"
    }
    this.loantypelist.push(LoanType2);
  }
  ...
  goldIn(stepper: MatStepper) {
    if (this.role == '1008') {
      this.submitted = true;
      if (this.goldloanForm.invalid) {
        return;
      }
      else if (this.goldid == null || this.goldid == undefined) {
        alert("Please Select Gold Ornaments")
      }
      else if (this.reasid == null || this.reasid == undefined) {
        alert("Please Select Gold Reason")
      }
      else if (this.disbid == null || this.disbid == undefined) {
        alert("Please Select Disbursement")
      }
      else if (this.ornamid == null || this.ornamid == undefined) {
        alert("Please Select Gold Ornaments")
      }
      else if ((this.Mon == true && this.Quar == true) || (this.Mon == false && this.Quar == false)) {
        alert("Please Select either Monthly or Quarterly Payment Mode")
      }
        ...
        let latest_date = this.datepipe.transform(this.DateTime, 'dd-MM-yyyy hh:mm:ss a');
        var cdata = {
          GoldType: this.goldid,
          ReasonId: this.reasid,
          Disbursement: this.disbid,
          Oranaments: this.ornamid,
          CustomerId: this.customerid,
          UserId: this.uid,
          requiredamount: this.requiredamount,
          overdrafts: this.overdrafts,
          goldamt: this.goldamt,
        ...
        this.businessloanservice.SaveGoldLoanDetails(cdata).subscribe((data: any) => {
          if (data == "success") {
            this.notify = "Gold Loan Details Saved Successfully!!"
            setTimeout(() => this.dismissalert = true, 100);
            setTimeout(function () {
              this.dismissalert = false;
            }.bind(this), 3000);
      ...
      if (this.goldid == null || this.goldid == undefined) {
        alert("Please Select Gold Ornaments")
      }
        ...
        this.businessloanservice.SaveGoldLoanDetails(data).subscribe((data: any) => {
          if (data == "success") {
            this.notify = "Gold Loan Details Saved Successfully!!"
            setTimeout(() => this.dismissalert = true, 100);
            setTimeout(function () {
  ...
  SavegoldDraft(stepper: MatStepper) {
    if (this.role == '1008') {
  ...
  GetDraftData() {
    ...
    this.businessloanservice.GetGoldLoanDraftdata(sendid).subscribe((data1: any) => {
      this.spinner.hide();
      ...
      else if (data.count == 4 && data.goldLoanDetails == null) {
        this.stepper1.selectedIndex = 4;

      }
      else if (data.count == 4 && data.goldLoanDetails != null) {
        this.goldid = data.goldLoanDetails.goldType,
          this.reasid = data.goldLoanDetails.reasonId,
          this.disbid = data.goldLoanDetails.disbursement,
          this.ornamid = data.goldLoanDetails.oranaments,
          this.requiredamount = data.goldLoanDetails.requiredamount,
          this.overdrafts = data.goldLoanDetails.overdrafts,
          this.goldamt = data.goldLoanDetails.goldamt,
          this.goldtotal = data.goldLoanDetails.goldtotal,
          this.goldvalue = data.goldLoanDetails.goldvalue,
          this.Mon = data.goldLoanDetails.mon,
          this.Quar = data.goldLoanDetails.quar,
          this.stepper1.selectedIndex = 4;
      }
  ...
  previewloandetails() {
    ...
    this.route.navigate(['previewloandetails/' + "gold"]);
  }
  submitloanform() {
    this.DateTime = new Date();
    let latest_date = this.datepipe.transform(this.DateTime, 'dd-MM-yyyy hh:mm:ss a');
